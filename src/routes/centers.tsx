...
export const Route = createFileRoute("/centers")({
  ...
  head: () => ({
    meta: [
      { title: "Browse Centers — Glowy" },
      { name: "description", content: "Browse and filter beauty, laser and aesthetic centers by city, service and rating." },
      { property: "og:title", content: "Browse Centers — Glowy" },
      { property: "og:description", content: "Find your next favorite beauty or laser clinic." },
    ],
  }),
  loader: ({ context, deps }) =>
...
const CATEGORIES = ["laser", "filler", "botox", "facial", "hair", "nails", "massage", "other"];
  ...
  return (
    <div className="min-h-screen flex flex-col">
      ...
      <div className="mx-auto w-full max-w-7xl px-4 py-12 sm:px-6">
        <h1 className="text-display text-5xl">All Centers</h1>
        <p className="text-muted-foreground mt-2">Filter by city, category and rating.</p>
        ...
        <p className="mt-6 text-sm text-muted-foreground">{data.centers.length} centers found</p>

        {data.centers.length === 0 ? (
          <div className="mt-8 rounded-2xl border border-dashed border-border bg-card p-16 text-center">
            <p className="text-display text-2xl text-muted-foreground">No centers match your filters.</p>
          </div>
        ) : (
