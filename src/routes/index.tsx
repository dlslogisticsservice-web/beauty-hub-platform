...
export const Route = createFileRoute("/")({
  head: () => ({
    meta: [
      ...
      { name: "description", content: "Discover and instantly book the best beauty, laser, filler and botox clinics in your city with Beauty Hub." },
...
function HomePage() {
  ...
  const handleSearch = () => {
    navigate({
      to: "/centers",
      search: { city: city || undefined, category: category || undefined } as never,
    });
  };
    ...
    <div className="min-h-screen flex flex-col bg-background">
            ...
            <div className="mt-8 rounded-2xl border border-border bg-card p-3 shadow-soft">
              <div className="flex flex-col gap-2 md:flex-row">
                ...
                <Button onClick={handleSearch} className="rounded-xl bg-gradient-primary px-6 shadow-soft">
                  <Search className="h-4 w-4 mr-2" /> {t("home.search_button")}
                </Button>
              </div>
            </div>
      ...
      <section className="mx-auto max-w-7xl px-4 pb-16 sm:px-6">
        <div className="flex items-end justify-between mb-8">
          ...
          <Link to="/centers" className="text-sm text-primary hover:underline">{t("home.browse_all")}</Link>
        </div>

        {data.centers.length === 0 ? (
          <div className="rounded-2xl border border-dashed border-border bg-card p-16 text-center">
            <p className="text-display text-3xl text-muted-foreground">{t("home.no_centers")}</p>
